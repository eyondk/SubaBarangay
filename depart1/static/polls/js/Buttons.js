...
function toggleButtons_Treasurer() {
    ...
    const deselectAllButton = document.getElementById("deselect-all");
    const deleteOneOrg = document.getElementById("delete-One-org"); // Fixed ID
    const deleteAllOrg = document.getElementById("delete-All-org"); // Fixed ID
    ...
    // Show "Delete" only when 1 row is selected
    deleteOneOrg.style.display = checkedBoxes.length === 1 ? "inline-block" : "none";

    // Show "Delete All" only when multiple rows are selected
    deleteAllOrg.style.display = checkedBoxes.length > 1 ? "inline-block" : "none";
}
...
function toggleButtons_Treasurer() {
    const checkboxes = document.querySelectorAll(".select-row-checkbox");
    const SaddButton = document.getElementById("S-add-org-all");
    const SupdateButton = document.getElementById("S-update-org-all");
    const SdeselectAllButton = document.getElementById("S-deselect-all");
    const SdeleteOneOrg = document.getElementById("S-delete-One-org"); // Fixed ID
    const SdeleteAllOrg = document.getElementById("S-delete-All-org"); // Fixed ID

    const checkedBoxes = Array.from(checkboxes).filter((checkbox) => checkbox.checked);

    SdeselectAllButton.style.display = checkedBoxes.length > 0 ? "inline-block" : "none";
    SupdateButton.style.display = checkedBoxes.length > 0 ? "inline-block" : "none";
    SaddButton.style.display = checkedBoxes.length > 0 ? "inline-block" : "none";

    // Show "Delete" only when 1 row is selected
    SdeleteOneOrg.style.display = checkedBoxes.length === 1 ? "inline-block" : "none";

    // Show "Delete All" only when multiple rows are selected
    SdeleteAllOrg.style.display = checkedBoxes.length > 1 ? "inline-block" : "none";
}
